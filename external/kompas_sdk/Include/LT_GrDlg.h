...
//------------------------------------------------------------------------------
// Вызов диалога "Выберете символ для точки". Более не используется
// ---
int LIB_FUNC ksExecDialPointStyleSelect( void *HWindow, int style );
...
#ifdef _UNICODE
#define ksExecQualityDialogT  ksExecQualityDialogW
#else
#define ksExecQualityDialogT  ksExecQualityDialog
#endif // !UNICODE 
...
//---
int LIB_FUNC  ksSelectD3Model( char        *name,       // буфер для имени файла
                               unsigned int bufLen,     // длина отведенного буфера name в символах
                               bool         onlyDetail, // только детали
                               bool         showAddNum );
...
//---
int LIB_FUNC  ksSelectD3ModelW( LPWSTR        name,       // буфер для имени файла
                                unsigned int  bufLen,     // длина отведенного буфера name в символах
                                bool          onlyDetail, // только детали
                                bool          showAddNum );

#ifdef _UNICODE
#define ksSelectD3ModelT  ksSelectD3ModelW
#else
#define ksSelectD3ModelT  ksSelectD3Model
#endif // !UNICODE
...
//---
int LIB_FUNC  ksGetSelectedEmbodimentMarking( char        *marking,       // буфер для обозначения исполнения
                                              unsigned int bufLen );      // Размер буфера
...
//---
int LIB_FUNC  ksGetSelectedEmbodimentMarkingW( LPWSTR        marking,       // буфер для обозначения исполнения
                                               unsigned int  bufLen );      // Размер буфера

#ifdef _UNICODE
#define ksGetSelectedEmbodimentMarkingT  ksGetSelectedEmbodimentMarkingW
#else
#define ksGetSelectedEmbodimentMarkingT  ksGetSelectedEmbodimentMarking
#endif // !UNICODE
...
//---
int LIB_FUNC  ksGetSelectedEmbodimentAdditionalNumber( char        *additionalNumber,  // буфер для дополниельного номера исполнения
                                                       unsigned int bufLen );          // Размер буфера
...
//---
int LIB_FUNC  ksGetSelectedEmbodimentAdditionalNumberW( LPWSTR       additionalNumber,  // буфер для дополниельного номера исполнения
                                                        unsigned int  bufLen );          // Размер буфера

#ifdef _UNICODE
#define ksGetSelectedEmbodimentAdditionalNumberT  ksGetSelectedEmbodimentAdditionalNumberW
#else
#define ksGetSelectedEmbodimentAdditionalNumberT  ksGetSelectedEmbodimentAdditionalNumber
#endif // !UNICODE
