...
type
  ...
  USERSELECTCALLBACKPROC = function( iEntity : PIEntity; iRequestInfo : PIRequestInfo ) : Integer; stdcall;
   // Функции обратной связи объектов созданных в тейк процессе
  USERTAKEOBJECTCALLBACKPROC = function( iEntity : PIUnknown ) : Integer; stdcall;
...
function ksCreateAttr3D( pObj   : PIUnknown;              // указатель объекта к которому подключается атрибут
                         ...
                         libname: PAnsiChar ) : PIAttribute3D;// имя библиотеки типов атрибутов
                         stdcall;
...
function ksDeleteAttr3D( pObj     : PIUnknown;            //  указатель объекта к которому подключен атрибут
                         ...
                         password : PAnsiChar ) : Integer;    //  пароль атрибута
                         stdcall;
...
function ksCreateAttr3DEx( pObj      : PIUnknown;                      // указатель объекта к которому подключается атрибут
                         ...
                         attr      : PksAttribute;                     // информация об атрибуте
                         attrID    : Double;                           // уникальный номер типа
                         libname   : PAnsiChar ) : PIAttribute3D; stdcall; // имя библиотеки типов атрибутов
