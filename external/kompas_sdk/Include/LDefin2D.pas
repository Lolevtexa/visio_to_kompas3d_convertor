...
//  '+-'  отмечены объекты, для которых реализован только GetObjParam
  ...
  ALLPARAM_W               = -9;  // все параметры объекта в СК объекта владельца для структур со строками wchar_t
  SHEET_ALLPARAM_W         = -10; // тоже что и  ALLPARAM  но параметры объекта в СК листа для структур со строками wchar_t
  VIEW_ALLPARAM_W          = -11; // тоже что и  ALLPARAM  но параметры объекта в СК вида для структур со строками wchar_t
...
// объекты см DrawingObjectTypeEnum
  ...
  CONIC_CURVE_4POINT_OBJ = 82;// Коническая кривая  по 4 точками и направлению +- Для GetObjParam используется структура NurbsParam
  CONIC_CURVE_5POINT_OBJ = 83;// Коническая кривая  по 5 точками               +- Для GetObjParam используется структура NurbsParam
...
// Определения для функции ksSytemPath
  ...
  sptCURR_APPS_FILES       = 16; // запоминание последних директориев, с которых выполнилось открытие/сохранение файла в диалоге Open|Save
  ...
  sptCUR_DOC_LIBS_FILES    = 23; // запоминание последних директориев, с которых выполнилось открытие/сохранение файла в диалоге Open|Save
  sptUTILS_FILES           = 24; // Выдать путь на каталог утилит
  sptMULTY_UTILS_FILES     = 25; // Выдать список каталогов утилит
  sptCURR_UTILS_FILES      = 26; // запоминание последних директориев, с которых выполнилось открытие/сохранение файла в диалоге Open|Save
  sptPROGRAMDATA_FILES     = 27; // Выдать путь на каталог данных приложения ( C:\ProgramData\ASCON\KOMPAS-3D\18 )
...
// oпределения флагов для текста  см ksTextItemEnum
  ...
  MARK_SEPARATOR     =  $18;   //разделитель в обозначении
...
// типы колонок для спецификации см ksSpecificationColumnTypeEnum
...
// ---
  ...
  MP_EXTERN_EDIT     = $04; //>0 интерфейс внешнего управления
  MP_PROPERTY_OBJECT = $08; //>0 интерфейс внешних свойств объекта
...
// типы документов DocType
  ...
  lt_DocTechnologyAssemble3D = 10;  // 10 - 3d-документ технологическая сборка
...
//enum ChangeOrderType
  ...
  co_BeforeObjectByGroupOrder = 6; // Перед объектом (расположить объекты группы в порядке их добавления в группу)
  co_AfterObjectByGroupOrder = 7;  // За объектом (расположить объекты группы в порядке их добавления в группу)
