...
//-----------------------------------------------------------------------------
procedure  StrResult( str   : PAnsiChar;             // строка сообщения
...
//------------------------------------------------------------------------------
function  SystemControlStart( menuCommand : PAnsiChar  ) : Integer; // указатель на строку,
...
//-----------------------------------------------------------------------------
function  FullFileName( oldName  : PAnsiChar;                        // старое имя файла
                        newName  : PAnsiChar;                        // буфер для полного имени файла
...
//-----------------------------------------------------------------------------
function  UniqueFileName( buff    :  PAnsiChar;                        // буфер для имени служебного файла
...
//-----------------------------------------------------------------------------
procedure RemoveUniqueFile( fileName : PAnsiChar ); stdcall;
...
//-----------------------------------------------------------------------------
function GetRightFileName( fileName : PAnsiChar;                        // буфер для имени файла
...
//---------------------------------------------------------------------------
function GetTextLength( text : PAnsiChar;            //строка текста
...
//  устройства - "<drive>:\")
//------------------------------------------------------------------------------------------------
function ksSystemPath( buff     : PAnsiChar;
                       buffLen  : Integer;
...
//  sptSYSTEM_FILES .. sptCURR_SHEETMETAL_FILES
//------------------------------------------------------------------------------------------------
procedure ksGetRelativePathFromSystemPath( sourcePath : PAnsiChar;     // полный путь к файлу
                                           destBuff   : PAnsiChar;     // (результат) относительный путь к файлу( без системного пути )
...
//  sptSYSTEM_FILES .. sptCURR_SHEETMETAL_FILES
//------------------------------------------------------------------------------------------------
procedure    ksGetFullPathFromSystemPath( relativePath : PAnsiChar;       // относительный путь к файлу( без системного пути )
                                          destBuff     : PAnsiChar;       // (результат) полный путь к файлу
                                          buffLen      : Integer;     // размер буффера
                                          pathType     : Integer );   // путь установленного типа см. ksSystemPath
                                          stdcall;
...
//------------------------------------------------------------------------------------------------
procedure  ksGetRelativePathFromFullPath( mainFilePath : PAnsiChar;      // полный путь к задающему файлу
                                          sourcePath   : PAnsiChar;      // полный путь к требуемому файлу
                                          destBuff     : PAnsiChar;      // (результат) относительный путь к требуемому файлу( без общей с задающим файлом части пути )
                                          buffLen      : Integer );  // размер буффера
                                          stdcall;
...
//------------------------------------------------------------------------------------------------
procedure ksGetFullPathFromRelativePath( mainFilePath : PAnsiChar;        //полный путь к задающему файлу
                                         relativePath : PAnsiChar;        //относительный путь к требуемому файлу( без общей с задающим файлом части пути )
                                         destBuff     : PAnsiChar;        //(результат) полный путь к требуемому файлу
                                         buffLen      : Integer );    // размер буффера
                                         stdcall;
...
//  iSize            = 29
//------------------------------------------------------------------------------------------------
function ksGetSystemProfileString( lpSection        : PAnsiChar;              //Имя секции
                                   lpKey            : PAnsiChar;              //Имя ключа
                                   lpReturnedString : PAnsiChar;              //Буфер для выходной строки
...
//-----------------------------------------------------------------------------------
procedure ksOpenHelpFile( helpFile : PAnsiChar;        // полное имя help-файла
...
// ---
function ksExecuteLibraryCommand( fileName : PAnsiChar; command : Integer ) : Integer; stdcall;
...
// ---
function ksIsLibraryEnabled( libName : PAnsiChar ) : Integer; stdcall; // имя библиотеки
...
// ---
function  ksGetQualityDefects( name : PAnsiChar; dimValue : Double;
...
//        0 - в единицах измерения текущего документа
// ---
function  ksGetQualityContensParam( name : PAnsiChar; param : QualityContensParam;
...
// ---
function  ksSetCurrentLibrary( libname : PAnsiChar; hModule : LongInt ) : Integer;  stdcall;
...
// ---
function  ksConvertLangStr( src :PAnsiChar; dst : PAnsiChar; dstMaxLen:integer ) : Integer;  stdcall;

//------------------------------------------------------------------------------
// Конвертировать строку src в dst в соответствии с текущим словарем
// ---
function  ksConvertLangStrW( src :PWideChar; dst : PWideChar; dstMaxLen:integer ) : Integer;  stdcall;
...
// Конвертировать окно с входящтми дочерними окнами в соответствии с текущим словарем
// ---
function  ksConvertLangWindowEx( hWnd_ : HWND; hInstance : LongWord; dlgID : PAnsiChar ) : Integer;  stdcall;
...
// Экспортный номер - 1025
//------------------------------------------------------------------------------
// Перерисовка части 2D документа ( Листа )
// Получает указатель на структуру RectParam и референс вида,
// в координатах которого задается RectParam
// ParamType == 0 Координаты в системе координат текущей СК Вида
// ParamType == VIEV_ALLPARAM  Координаты в системе координат Вида
// ParamType == SHEET_ALLPARAM Координаты в системе координат чертежа
// ---
function ksReDrawDocPartEx( rect      : RectParam;  // Указатель на структуру прямоугольника
                            pView     : Reference;  // референс вида 2D документа( Листа )
                            ParamType : Integer ) : Integer;  stdcall;
...
// ---
function ksCalcRasterScale( const fileName : PAnsiChar; w, h : Double ) : Double; stdcall;
...
// ---
function ksFileNameCompare( fileName1, fileName2 : PAnsiChar ) : Integer; stdcall;
...
//-----------------------------------------------------------------------------
function ksChoiceFile( ext     : PAnsiChar;                    // расширение имени файла
                       filter  : PAnsiChar;                    // фильтр поиска (0 - формируется автоматически)
                       name    : PAnsiChar;                    // буфер для имени файла
...
// В этом случае name можно получить функцией GetRightFileName.
//-----------------------------------------------------------------------------
function ksChoiceFileAppointedDir( ext     : PAnsiChar;             // расширение имени файла
                                   filter  : PAnsiChar;             // фильтр поиска (0 - формируется автоматически)
                                   name    : PAnsiChar;             // буфер для имени файла
                                   bufLen  : Cardinal;          // размер отведенного буфера name
...
//-----------------------------------------------------------------------------
function ksChoiceFiles( ext     : PAnsiChar;                   // расширение имени файла
                        filtr   : PAnsiChar;                   // фильтр поиска (0 - формируется автоматически)
...
// В этом случае name можно получить функцией GetRightFileName.
//-----------------------------------------------------------------------------
function  ksSaveFile( ext     :  PAnsiChar;                     // расширение имени файла
                      oldName :  PAnsiChar;                     // имя файла по умолчанию
                      filter  :  PAnsiChar;                     // фильтр поиска (0 - формируется автоматически)
                      name    :  PAnsiChar;                     // буфер для имени файла
                      bufLen  :  Cardinal;                  // размер отведенного буфера name
                      preview :  Byte ) : Cardinal; stdcall; // 1-с подключением окна предварительного просмотра, 0-без
...
// Выдать сообщение.
//-----------------------------------------------------------------------------
procedure ksMessage( s : PAnsiChar ); stdcall;// s - строка сообщения
...
//-----------------------------------------------------------------------------
procedure  Error( Msg : PAnsiChar ); stdcall; // строка сообщения об ошибке
...
//-----------------------------------------------------------------------------
function  YesNo( s : PAnsiChar ) : Integer; stdcall; // s - строка сообщения
...
function  ksDrawSlideFromFile( HWindow : HWND;                          // HWND окна
                               fileName   : PAnsiChar ) : Integer; stdcall; // идентификатор BMP в файле ресурсов приложения
...
// Возвращается 1 в случае успеха, 0 в случае неудачи.
//-----------------------------------------------------------------------------
function WriteSlide( filename : PAnsiChar;          // имя файла для записи
...
// заданный интервал.
//-----------------------------------------------------------------------------
function   ReadDouble( mess      : PAnsiChar;          // строка приглашения
...
//-----------------------------------------------------------------------------
function   ReadInt( mess      : PAnsiChar;                       // строка приглашения
...
//-----------------------------------------------------------------------------
function   ReadString( smess  :  PAnsiChar;                       // строка приглашения
                       str    :  PAnsiChar;                       // возвращаемая строка
...
// заданный интервал.
//-----------------------------------------------------------------------------
function ReadLong( mess      : PAnsiChar;                         // строка приглашения
...
//                      | 16 - Сетки      |            |               |                   |
//      ----------------+-----------------+------------+---------------+-------------------+-------------------------
function  ksMaterialDlg( HWindow : Pointer; material : PAnsiChar; sizeStr : Integer; Var plt : Double; Var kod_sizeLib : Double4; kod_tip : PAnsiChar ) : Integer;  stdcall;

//------------------------------------------------------------------------------
// Получение предопределенного текста из файла *.pdt
// Функция возвращает 1 в случае удачного завершения
// ---
function  ksExecDialPredefinedText( HWindow : Pointer; str : PAnsiChar; sizeStr : Integer ) : Integer;  stdcall;
...
// ---
function ksEditTextLine( HWindow : HWND; str : PAnsiChar;
...
// ---
procedure ksStartProgressBar( minVal, maxVal : LongInt; newText : PAnsiChar; resetText : Integer ); stdcall;
...
//  resetText - TRUE - обновить текст в строке состояния
// ---
procedure ksSetProgressBar( currentVal : LongInt; newText : PAnsiChar; resetText : Integer ); stdcall;
...
//  newText- текст в строке состояния
// ---
procedure ksSetProgressText( newText : PAnsiChar ); stdcall;
...
//  resetText - TRUE - обновить текст в строке состояния
// ---
procedure ksStopProgressBar( newText : PAnsiChar; resetTxt : Integer ); stdcall;
...
function SetMacroParam( ref      : reference;          // указатель на макроэлемент
                        ...
                        fileName : PAnsiChar;              // имя файла библиотеки, в которой находтся функция редактирования
                        libName  : PAnsiChar;              // имя библиотеки, в которой находтся функция редактирования
...
function  ksGetMacroEditParam( ref          : Reference;        // указатель на макроэлемент
                               fileName     : PAnsiChar;            // указатель на буфер имени файла библиотеки
                               fileNameSize : Cardinal;         // размер буферa имени файла библиотеки
                               libName      : PAnsiChar;            // указатель на буфер имени библиотеки
...
//-------------------------------------------------------------------------------------
function ksGetLibraryStylesArray( libraryName : PAnsiChar;               // полное имя библиотеки стилей
...
function SaveGroup( g    : Reference;  // указатель группы
                    name : PAnsiChar ) : integer; //имя сохраняемой группы
                    stdcall;
...
//--------------------------------------------------------------------------
function GetGroup( name : PAnsiChar ) : Reference; //имя группы
                   stdcall;
...
function ksGetGroupName( gr    : Reference;                        // указатель на группу
                         name : PAnsiChar; size : Integer ) : Integer; // указатель строки для имени группы и размер строки
                         stdcall;
...
//  сколько угодно.
//--------------------------------------------------------------------------
function  FragmentDefinition( fileName   : PAnsiChar;             //имя файла фрагмента
                              comment    : PAnsiChar;             //имя вставки
...
//--------------------------------------------------------------------------
function ReadFragment( fileName    : PAnsiChar;                       // имя фрагмента
                       curentLayer : Byte;                        //тип размещения по слоям 0 - на свои слои 1- в текущий слой
...
//  берется из библиотеки фрагментов
//--------------------------------------------------------------------------
function  ksReadFragmentToGroup( fileName    : PAnsiChar;                        // имя фрагмента
                                 curentLayer : Byte;                         // тип размещения по слоям 0 - на свои слои 1- в текущий слой
                                 par         : PPlacementParam ) : Reference;// параметры привязки
                                 stdcall;
...
//--------------------------------------------------------------------------
function  ksReadFragmentToGroupEx( fileName  : PAnsiChar;                     // имя фрагмента
                                 curentLayer : Byte;                      // тип размещения по слоям 0 - на свои слои 1- в текущий слой
                                 par         : PPlacementParam;
...
function WriteFragment( gr       : Reference;          //указатель на группу
                        filename : PAnsiChar;              //имя файла фрагмента
                        comment  : PAnsiChar;              //комментарий для фрагмента
...
//-------------------------------------------------------------------------
function LocalFragmentDefinition( comment : PAnsiChar ) : Integer; //имя вставки
                                  stdcall;
...
//----------------------------------------------------------------------------------------------
function ksChoiceFragmentFromLib( frwLibFile : PAnsiChar;            // полное имя библиотеки фрагментов
                                  frwName    : PAnsiChar;            // имя фрагмента
...
//----------------------------------------------------------------------------------------------
function ksFragmentLibrary( libName : PAnsiChar;                //имя файла библиотеки фрагментов
...
//         4 - минимизировать окно библиотекаря
//----------------------------------------------------------------------------------------------
function ksAddFragmentToLibrary ( libName : PAnsiChar;             // имя файла библиотеки фрагментов
                                  frwName : PAnsiChar ) : Integer; // имя файла фрагмента
                                  stdcall;
...
//----------------------------------------------------------------------------------------------
function ksCheckFragmentLibrary( libName         : PAnsiChar;             // имя файла библиотеки фрагментов
...
// ---
function ksExistFragmentInLibrary( frwName : PAnsiChar ) : Integer; stdcall;
...
//---------------------------------------------------------------------------
function ksGetLibraryTreeStruct( libName : PAnsiChar; root : PTreeNodeParam ) : Integer; stdcall;
...
//----------------------------------------------------------------------
function OpenDocument( nameDoc : PAnsiChar;              //- имя документа
...
function SaveDocument ( sheet    : Reference;         // указатель документа
                        fileName : PAnsiChar ) : Integer; // имя файла документа для режима Safe as... или NULL
                        stdcall;
...
//             1 - в версию  5.11
//----------------------------------------------------------------------
function SaveDocumentEx( sheet : Reference; fileName : PAnsiChar; saveMode : Integer ) : Integer; stdcall;
...
function ksSaveAsToRasterFormat ( sheet    : Reference;         // указатель документа
                                  fileName : PAnsiChar;             // имя файла документа
...
function ksDrawKompasDocument( HWindow     : HWND;              // несущее окно
                               docFileName : PAnsiChar ) : Integer; // полное имя файла документа
                               stdcall;
...
//---------------------------------------------------------------------------
function ksReadTableFromFile( tblFileName : PAnsiChar ) : Reference; //полное имя к файлу таблицы
                              stdcall;
...
function ksGetZona( x, y : Double;              // x,y - координаты точки в текущем документе;
                    zona : PAnsiChar;               // буфер для возврата зоны;
...
//------------------------------------------------------------------------------------------------
//  растянуть изображение активного окна на весь экран, ограниченное прямоугольником с центром
//  и масштабом . viewParam = true - Координаты центра в СК вида, viewParam = false - Координаты центра в СК листа
//------------------------------------------------------------------------------------------------
function ksSetZoomScale ( x, y, scale : Double; viewParam : Integer ) : Integer; stdcall;
...
// ---
function ksPrintKompasDocument( fileName : PAnsiChar; toFile : PAnsiChar;
...
// Функция возвращает 0 в случае неудачи.
// ---
function ksPrintKompasDocumentEx( fileName : PAnsiChar; toFile : PAnsiChar;
...
// 0 в случае неудачи
//-------------------------------------------------------------------------------
function ksGetDocumentTypeByName( fileName : PAnsiChar ) : Integer; stdcall;
...
function TextLine( bitVector : Cardinal;          //свойства строки, которые задаются вкл.-выкл.
                   ...
                   s         : PAnsiChar ) : Integer; //строка символов
                   stdcall;
...
function ksText( x, y,                             //точка привязки текста
                 ...
                 s         : PAnsiChar ) : Reference;  //строка символов
                 stdcall;
...
procedure  IntersectArcLin( xc, yc, rad,                     // координаты центра и радиус дуги
                            ...
                            Var kp : Integer;                // количество пересечений
                            Var xp, yp : Double2 );  stdcall;  // коор-ты точек пересечения
...
procedure IntersectCirCir( xc1, yc1,            //центр первой окружности
                           ...
                           radius2 : Double;    //радиус втророй окружности
			                     Var kp  : Integer;   //количество точек пересечения
                           Var xp, yp  : Double2 ); //массивы точек пересечения
                           stdcall;
...
procedure TanLinePointCircle( x, y,                   // координаты точки
                              ...
                              Var xp, yp : Double2 ); // массивы точек касания
                              stdcall;
...
// Экспортный номер - 1033
//-----------------------------------------------------------------------------
// Функция расчета касательной к кривой.
// ---
procedure TanLineAngCurve(  p        : Reference;   // указатель на кривую
                            ang      : Double;      // угол касательной
                            pointArr : Reference ); // массив точек касания
                            stdcall;
...
procedure CouplingLineLine( x1, y1,           //точка на первой прямой
                            ...
                            Var con: CON4 );  //структуры сопряжения
                            stdcall;
...
procedure ksCouplingLineCircle( xc, yc,           //центр окружности
                                ...
                                rad    : Double;  //радиус окружности сопряжения
                                Var kp : Integer; //количество сопрягающих окружностей
                                Var con: CON8 );  //структуры сопряжения
                                stdcall;
...
//------------------------------------------------------------------------------
// Функция возвращает угол нормали к кривой в заданной точке по параметру кривой ( 0 - 360 гр ).
// В случае неудачи возвращается -1
// ---
function ksGetCurvePerpendicularByT( curve : Reference;          // указатель на кривую
                                     t     : Double ) : Double;  // параметр кривой
                                     stdcall;
...
// Функция возвращает 1 в случае успеха или 0 при неудаче.
// ---
function ksMovePointOnCurveEx( curve   : Reference;           // указатель на кривую
                               Var x, y,                      // координаты точки
                               t       : Double;              // параметр кривой
                               len     : Double;              // расстояние на которое нужно сместить точку
                               dir     : Integer;             // направление продвижения точки(1 - в направлении построения кривой, -1 в обратном напрвлении )
                               ext     : Integer ) : Integer; // 1 - Если кривая не замкнута и длина ее части от точки до конца в заданном направлении меньше,
                               stdcall;                       //     чем нужное смещение, то вычислить значение на продолжении кривой, если можно построить продолжение.
...
// Функция возвращает  1 в случае успеха или 0 в случае неудачи
//--------------------------------------------------------------------------
function ksCalculate( s       : PAnsiChar;              //выражение
...
// Экспортный номер - 1104
//--------------------------------------------------------------------------------
//   Расстояние между двумя кривыми.
//   Возвращает:
//   1 успешное завершение
//   0 кривые пересекаются или совпадают
//   -1 первый объект не существует
//   -2 второй объект не существует
//   -3 кривые расположены в разных видах
//   -4 не совпадают СК определения кривых (геом и анн) (?)
...
//   -7 ошибка
//---------------------------------------------------------------------------------
function ksDistanceCurveCurve( p1 : Reference;            // указатель на первую кривую
                               p2 : Reference;            // указатель на вторую кривую
                               var distanse : Double;    // растояние
                               var t1 : Double;
                               var t2 : Double  ) : Integer;       // параметр на кривой 2 в точке с минимальным удалением
                               stdcall;
...
function ksCreateAttrType( attrType : PksAttributeType; // информация о типе атрибута
                           libname  : PAnsiChar ) : Double; // имя библиотеки типов атрибутов
                           stdcall;
...
function DeleteAttrType( attrID   : Double;               // уникальный номер типа
                         libname  : PAnsiChar;                // имя библиотеки типов атрибутов
                         password : PAnsiChar   ) : Integer;  // пароль  типа атрибута
                         stdcall;
...
function ksSetAttrType(  attrID   : Double;           // уникальный номер типа
                         ...
                         password : PAnsiChar ) : Double; // пароль типа атрибута
                         stdcall;
...
function CreateAttr( pObj    : Reference;           // указатель объекта к которому подключается атрибут
                     ...
                     libname : PAnsiChar ) : Reference; // имя библиотеки типов атрибутов
                     stdcall;
...
function ViewEditAttr (  pAttr    : Reference;         // указатель атрибута
                         ...
                         password : PAnsiChar ) : Integer; // пароль атрибута  для просмотра 0
                         stdcall;
...
//  Диалог привязан к главному окну.
//-------------------------------------------------------------------------
function ViewEditAttrType ( libname : PAnsiChar;              // имя библиотеки типов атрибутов
                            _type   : Cardinal;           // 1- просмотр 2- редактирование
                            attrId  : Double;             // уникальный номер типа
                            password: PAnsiChar ) : Integer;  // пароль типа для просмотра 0
                            stdcall;
...
// Диалог привязан к главному окну.
//-------------------------------------------------------------------------
function ChoiceAttrTypes ( libname : PAnsiChar  ) : Double;  // имя библиотеки типов атрибутов
                           stdcall;
...
// В случае неудачи возвращает 0.
//-------------------------------------------------------------------------------------
function ksGetLibraryAttrTypesArray( libname : PAnsiChar ): Reference;    // полное имя библиотеки типов атрибутов
                                     stdcall;
...
//------------------------------------------------------------------------------------------------
function ksSpcObjectCreate( nameLib             : PAnsiChar;            //имя библиотеки стилей
...
// reference spsObj = ksSpcObjectEnd();
//------------------------------------------------------------------------------------------------
function ksSpcMassa ( sMassa : PAnsiChar ) : Integer;        //масса детали
                      stdcall;
...
function ksSpcCount ( ispoln : Byte;              //номер исполнения
                      sCount : PAnsiChar ) : Integer; //количество деталей
                      stdcall;
...
//------------------------------------------------------------------------------------------------
function ksGetSpcObjForGeom( nameLib : PAnsiChar;               //имя библиотеки типов спецификации
...
// только для графических документов
//------------------------------------------------------------------------------------------------
function ksGetSpcObjForGeomWithLimit( nameLib      : PAnsiChar;                //имя библиотеки типов спецификации
                                      numb         : Cardinal;             //номер типа спецификации
                                      obj          : Reference;            // указатель на геометрию -группа или объект
                                      equal        : Byte;                 // 1 - присланная геометрия идентична геометрии объекта СП; 0- входит в объект СП
...
function  ksGetSpcSectionName( spcObj : Reference;           // референс обеъекта спецификации
                               name   : PAnsiChar;           // буфер для имени раздела
...
// в спецификации не используются
//----------------------------------------------------------------------
function CreateSpcIterator( nameLib    : PAnsiChar;                   //имя библиотеки стилей спецификации
                            styleNumb  : Cardinal;                //номер стиля спецификации
...
//-------------------------------------------------------------------------------------------------
function ksGetSpcTableColumn( nameLib        : PAnsiChar;            //имя библиотеки типов
                              numb           : Cardinal;         // номер типа спецификации
...
// ---
function ksGetSpcStyleParam( nameLib : PAnsiChar;       // имя библиотеки типов
                             numb    : Cardinal;    // номер типа спецификации
...
function ksGetSpcDescription( pDoc  : Reference;          // указатель на документ
                              ...
                              state : PAnsiChar ) : Integer;  // состояние: true - активное, false - неактивное
                              stdcall;
...
// 0 в случае ошибки
// ---
function ksGetSpcObjectSummaryCountText( spcObj : Reference;     // объект спецификации
                                     ispoln,                 // номер колонки типа "количество", начиная с 1( актуально для СП типа Б )
                                     blockNumber : Integer ) // номер блока, начиная с 0 ( актуально для СП типа A )
                                     : Reference; stdcall;
...
// ---
function ksSetSpcObjectMaterial( spcObj : Reference; material : PAnsiChar; density : double ) : Integer; stdcall;
...
// HWindow  - HWND окна
//--------------------------------------------------------------------------
function ksViewGetDensityAndMaterial( material : PAnsiChar;
                                      sizeStr  : Integer;
                                      HWindow  : HWND ) : Double;
                                      stdcall;
...
function ksSetSpcPerformanceName( index,	                           // индекс описания СП в документе
                                  ispoln,                            // номер колонки типа "количество", начиная с 1( актуально для СП типа Б )
                                  blockNumber : Integer;             // номер блока, начиная с 0 ( актуально для СП типа A )
                                  name        : PAnsiChar) : Integer;    // имя исполнения
                                  stdcall;
...
function ksDrawKompasText( HWindow : HWND;                        // несущее окно
                           text    : PAnsiChar ): Integer;  stdcall;  // Текст
...
// ---
function ksExecDialSymbol( HWindow : HWND; var symb : Integer; font : PAnsiChar; len : Integer ) : Integer; stdcall;
...
//  Вызов диалога "Вставка символа"
// ---
function ksExecDialSymbolW( HWindow : HWND; var symb : Integer; font : PWideChar; len : Integer ) : Integer;   stdcall;
...
//-------------------------------------------------------------------------------------------------
function ksSpcDocLinksClearEx( pDoc : Reference; mode : Integer ) : Integer; stdcall;
...
// ---
function ksExecDialPointStyleSelect( HWindow : HWND; style : Integer ) : Integer; stdcall;
...
//---
function ksSetCursorText( text : PAnsiChar ) : Integer; stdcall;
...
// Экспортный номер - 1043
//-------------------------------------------------------------------------------
// Список ошибок после открытия файла с восстановлением
// ---
function ksGetRecoverModeErrorList( var errors : OleVariant ) : Integer; stdcall;
...
// Экспортный номер - 1034
//-----------------------------------------------------------------------------
// Создать вставку фрагмента
//  fileName - полное имя файла фрагмента.
//  Допускается fileName  следующего вида "c:\_f\gr\lib1.lfr|детали|литье|фланец"
//  ( c:\_f\gr\lib1.lfr - имя файла библиотеки фрагментов, |детали|литье|- разделы, подразделы
//  внутри библиотеки фрагментов, фланец - имя фрагмента ). В этом случае определение
//  создается на фрагмент из библиотеки фрагментов.
// ---
function ksCreateInsertionFragment( fileName : PAnsiChar ) : Integer; stdcall;

// Экспортный номер - 1035
//-------------------------------------------------------------------------------
// Создать вставку фрагмента
//  fileName - полное имя файла фрагмента.
//  Допускается fileName  следующего вида "c:\_f\gr\lib1.lfr|детали|литье|фланец"
...
//  создается на фрагмент из библиотеки фрагментов.
// ---
function ksCreateInsertionFragmentW( fileName : PWideChar ) : Integer; stdcall;

// Экспортный номер - 1033
//------------------------------------------------------------------------------
// Проверить, является ли версия учебной
//---
function ksIsStudyVersion() : Integer; stdcall;
...
// Экспортный номер - 1037
//------------------------------------------------------------------------------
// Проверить, доступность продукта по номеру
//---
function ksIsLibraryNumberAvailable( prodNumb : Cardinal ) : Integer; stdcall;

// Экспортный номер - 1038
//------------------------------------------------------------------------------
// Проверить, используется ли версия Компас - Строитель
//---
function ksIsSpdsVersion() : Integer; stdcall;

// Экспортный номер - 1039
//------------------------------------------------------------------------------
// Вернуть версию приложения Компас
// Компас - Строитель, Компас - Студент, Иностранная версия и т.к.
//---
function ksKompasVariant() : Integer; stdcall;

// Экспортный номер - 1040
//-----------------------------------------------------------------------------
// Функция расчета касательных к кривой проходящих через заданную точку.
// ---
procedure ksLinePointTangentCurve(  p      : Reference;   // указатель на кривую
                                    xc, yc : Double;      // координаты точки
                                    angArr : Reference ); // массив углов касательных


//------------------------------------------------------------------------------
// Выбор квалитета
// ---
function ksExecQualityDialog( HWindow : HWND;
                              curQual : PAnsiChar;
                              var dimValue : Double;
                              inMM : SmallInt;
                              param :  PQualityContensParam ) : Integer; stdcall;


//------------------------------------------------------------------------------
// Выбор квалитета (Unicode)
// ---
function ksExecQualityDialogW( HWindow : HWND;
                                   curQual : PWideChar;
                                   var dimValue : Double;
                                   inMM : SmallInt;
                                   param : QualityContensParamW ) : Integer; stdcall;


//-----------------------------------------------------------------------------
// Установить тип подсветки объекта (light=1 - красный) или (light=0 - зеленый) подсветку для объекта
// с указателем ref (объект вида, вид, группа, слой).
// Возвращает 1 в случае успеха, 0 в случае неудачи.
//-----------------------------------------------------------------------------
function ksSetLightObjType( ref : Reference;         // указатель на объект
                            light : Byte ) : Integer; stdcall; // режим подсветки


//-----------------------------------------------------------------------------
// Скрыть показать временный объект в документе
// с указателем ref (временный объект).
// Возвращает 1 в случае успеха, 0 в случае неудачи.
//-----------------------------------------------------------------------------
function ksShowHideTmpObj( ref : Reference;                      // указатель на объект
                           show : Integer ) : Integer; stdcall;  // ksShowHideTmpObjTypeEnum скрыть-показать, показать как фантом

// Экспортный номер - 1024
//-------------------------------------------------------------------------------
// Получить текущую информацию о привязках
// ---
function ksGetSnapInfo() : PISnapInfo; stdcall;

// Экспортный номер - 1027
//-------------------------------------------------------------------------
//  Выдать информацию о типе атрибута.
//  Если присылается NULL, соответствующее поле не заполняется.
//-------------------------------------------------------------------------
function ksGetAttrTypeInfo( pAttr                  : Reference;   // указатель атрибута
                            var numb               : Double;      // номер типа атрибута
                            attrTypeName           : PAnsiChar;   // Имя типа аттрибута
                            attrTypeNameSize       : Integer;     // размер выделенной памяти под attrTypeName
                            attrTypeFileName       : PAnsiChar;   // Имя файла где описан тип аттрибута
                            attrTypeFileNameSize   : Integer;     // размер выделенной памяти под attrTypeFileName
                            attrTypePathInFile     : PAnsiChar;   // Путь к типу внутри библиотеки
                            attrTypePathInFileSize : Integer ) : Integer; stdcall; // размер выделенной памяти под attrTypePathInFile

// Экспортный номер - 1028
//-------------------------------------------------------------------------
//  Выдать информацию о типе атрибута.
//  Если присылается NULL, соответствующее поле не заполняется.
//-------------------------------------------------------------------------
function ksGetAttrTypeInfoW( pAttr                 : Reference;   // указатель атрибута
                            var numb               : Double;      // номер типа атрибута
                            attrTypeName           : PWideChar;   // Имя типа аттрибута
                            attrTypeNameSize       : Integer;     // размер выделенной памяти под attrTypeName
                            attrTypeFileName       : PWideChar;   // Имя файла где описан тип аттрибута
                            attrTypeFileNameSize   : Integer;     // размер выделенной памяти под attrTypeFileName
                            attrTypePathInFile     : PWideChar;   // Путь к типу внутри библиотеки
                            attrTypePathInFileSize : Integer ) : Integer; stdcall; // размер выделенной памяти под attrTypePathInFile

// Экспортный номер - 1044
//-----------------------------------------------------------------------------
// Выбрать из списка открытых или из файла модели
//---
function  ksSelectD3Model( name       : PAnsiChar;       // буфер для имени файла
                           bufLen     : Integer;     // длина отведенного буфера name в символах
                           onlyDetail : Boolean; // только детали
                           showAddNum : Boolean ) : Integer; stdcall;


// Экспортный номер - 1045
//-----------------------------------------------------------------------------
// Выбрать из списка открытых или из файла модели (Unicode)
//---
function  ksSelectD3ModelW( name       : PWideChar;  // буфер для имени файла
                            bufLen     : Integer;    // длина отведенного буфера name в символах
                            onlyDetail : Boolean;    // только детали
                            showAddNum : Boolean ) : Integer; stdcall;
...
//---
function  ksGetSelectedEmbodimentMarking( marking : PAnsiChar;       // буфер для обозначения исполнения
                                          bufLen  : Integer ) : Integer; stdcall;      // Размер буфера



//-----------------------------------------------------------------------------
// Возвращает обозначение исполнения выбранное в диалоге выбора файла (ksSelectD3Model) (Unicode)
//---
function  ksGetSelectedEmbodimentMarkingW( marking : PWideChar;       // буфер для обозначения исполнения
                                           bufLen  : Integer ) : Integer; stdcall;      // Размер буфера
...
//---
function  ksGetSelectedEmbodimentAdditionalNumber( additionalNumber : PAnsiChar;  // буфер для обозначения исполнения
                                                   bufLen           : Integer ) : Integer; stdcall;          // Размер буфера



//-----------------------------------------------------------------------------
// Возвращает дополнительный номер исполнения выбранный в диалоге выбора файла (ksSelectD3Model) (Unicode)
//---
function  ksGetSelectedEmbodimentAdditionalNumberW( additionalNumber : PWideChar;  // буфер для обозначения исполнения
                                                    bufLen : Integer ) : Integer; stdcall;          // Размер буфера
...
// ---
function ksGetObjectsNameByType( type_ : Integer; name : PAnsiChar; bufLen : Integer ) : Integer; stdcall;


//-----------------------------------------------------------------------------
// вернуть имя объекта по его типу
// ---
function ksGetObjectsNameByTypeW( type_ : Integer; name : PWideChar; bufLen : Integer ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// вернуть имя объекта по его типу
// ---
function ksGetObjectNameByType( type_ : Integer; name : PAnsiChar; bufLen : Integer ) : Integer; stdcall;


//-----------------------------------------------------------------------------
// вернуть имя объекта по его типу
// ---
function ksGetObjectNameByTypeW( type_ : Integer; name : PWideChar; bufLen : Integer ) : Integer; stdcall;
...
// ---
function  ksGetSpcPropertyFill( spcObj : Reference ) : LongInt; stdcall;

// Экспортный номер - 1055
//-------------------------------------------------------------------------------
//  Взвести флаг Синхронизировать со свойствами компонента
// ---
function  ksSetSpcPropertyFill( spcObj : Reference; val : LongInt ) : Integer; stdcall;

// Экспортный номер - 1100
//---------------------------------------------------------------------------
// Установить параметры материала для диалога МЦХ
// material - Наименование материала
// density  - плотность
// Возвращает 1 в случае успеха и 0 в случае неудачи
//---------------------------------------------------------------------------
function ksSetMixDlgMaterialParam( material : PAnsiChar; density : Double ) : Integer; stdcall;

// Экспортный номер - 1101
//---------------------------------------------------------------------------
// Установить параметры материала для диалога МЦХ
// material - Наименование материала
// density  - плотность
// Возвращает 1 в случае успеха и 0 в случае неудачи
//---------------------------------------------------------------------------
function ksSetMixDlgMaterialParamW( material : PWideChar; density : Double ) : Integer; stdcall;

//-------------------------------------------------------------------------------
// Текущая версия записи документов
// Получать требуется в событии начала записи документа
//-------------------------------------------------------------------------------
function ksGetDocumentSaveVersion() : Integer; stdcall;

//-------------------------------------------------------------------------------
// Версия файла с которой документ был сохранен
//-------------------------------------------------------------------------------
function ksGetDocumentOpenVersion( docRef : Reference ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// запретить/разрешить продукт (занять лицензию)
// ---
function ksSetLibraryEnable( prodNumb : Cardinal;
                             enable   : Byte ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// Получить состояние защиты продукта
// ---
function ksGetLibraryStatus( prodNumb : Cardinal ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// Ознакомительный период
// ---
function ksIsLibraryTrial( prodNumb : Cardinal )  : Integer; stdcall;

//-----------------------------------------------------------------------------
// Признак локальный/сетевой продукт
// ---
function ksIsLibraryLocal( prodNumb : Cardinal ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// Признак локальный/сетевой продукт
// ---
function ksIsLibraryProductName( prodNumb : Cardinal; productName : PAnsiChar; nameLen : Integer ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// Получить название продукта (Unicode)
// ---
function ksIsLibraryProductNameW( prodNumb : Cardinal; productName: PWideChar; nameLen : Integer ) : Integer; stdcall;


//-----------------------------------------------------------------------------
// Получить информацию о текущей сессии
// ---
function ksIsLibraryProductKeyInfo( prodNumb : Cardinal; keyInfo : PAnsiChar; keyInfoLen : Integer ) : Integer; stdcall;

//-----------------------------------------------------------------------------
// Получить информацию о текущей сессии (Unicode)
// ---
function ksIsLibraryProductKeyInfoW( prodNumb   : Cardinal;
                                     keyInfo    : PWideChar;
                                     keyInfoLen : Integer ) : Integer; stdcall;


//-----------------------------------------------------------------------------
// Разрешена ли печать
// ---
function ksIsPrintAvailable() : Integer; stdcall;

//-----------------------------------------------------------------------------
// Разрешен ли экспорт в другие форматы
// ---
function ksIsExportAvailable() : Integer; stdcall;

//-----------------------------------------------------------------------------
// Зарегистрировать номер продукта на сервере лицензий Компас
// ---
function ksRegisterLibraryNumber( prodNumb : Cardinal ) : Cardinal;  stdcall;

//-----------------------------------------------------------------------------
// Разрешен ли экспорт в другие форматы
// ---
function ksUnRegisterLibraryNumber( prodNumbUnicueId : Cardinal ) : Integer; stdcall;
...
function  ksSheetSetupDlg; external APPTOOL5 Name 'PascalSheetSetupDlg';
...
function ksConvertLangStrW; external APPTOOL5 Name 'ksConvertLangStrW';
...
function ksSheetSetupDlgW; external APPTOOL5 Name 'PascalSheetSetupDlgW';
...
function ksSpcDocLinksClearEx; external APPTOOL5 Name 'ksSpcDocLinksClearEx';
function ksGetSnapInfo; external APPTOOL5 Name 'ksGetSnapInfo';
function ksReDrawDocPartEx; external APPTOOL5 Name 'ksReDrawDocPartEx';

// функции появившиеся в Компас14
function ksExecDialPointStyleSelect; external APPTOOL5 Name 'ksExecDialPointStyleSelect';
function ksGetAttrTypeInfo; external APPTOOL5 Name 'ksGetAttrTypeInfo';
function ksGetAttrTypeInfoW; external APPTOOL5 Name 'ksGetAttrTypeInfoW';


// функции появившиеся в Компас15
procedure IntersectCurvCurvEx; external APPTOOL5 Name 'IntersectCurvCurvEx';
function ksIntersectCurvCurvEx; external APPTOOL5 Name 'ksIntersectCurvCurvEx';
function ksIsStudyVersion; external APPTOOL5 Name 'ksIsStudyVersion';

// Компас 15 Sp1
procedure TanLineAngCurve; external APPTOOL5 Name 'TanLineAngCurve';
function ksCreateInsertionFragment; external APPTOOL5 Name 'ksCreateInsertionFragment';
function ksCreateInsertionFragmentW; external APPTOOL5 Name 'ksCreateInsertionFragmentW';
function ksIsLibraryNumberAvailable ; external APPTOOL5 Name 'ksIsLibraryNumberAvailable';

function ksIsSpdsVersion; external APPTOOL5 Name 'ksIsSpdsVersion';
function ksKompasVariant; external APPTOOL5 Name 'ksKompasVariant';

// функции появившиеся в Компас15 SP2
procedure ksLinePointTangentCurve; external APPTOOL5 Name 'ksLinePointTangentCurve';

// функции появившиеся в Компас16
function ksMovePointOnCurveEx; external APPTOOL5 Name 'ksMovePointOnCurveEx';
function ksGetCurvePerpendicularByT; external APPTOOL5 Name 'ksGetCurvePerpendicularByT';
function ksGetRecoverModeErrorList; external APPTOOL5 Name 'ksGetRecoverModeErrorList';

// функции появившиеся в Компас16.1
function ksSelectD3Model; external APPTOOL5 Name 'ksSelectD3Model';
function ksSelectD3ModelW; external APPTOOL5 Name 'ksSelectD3ModelW';

function ksExecQualityDialog; external APPTOOL5 Name 'ksExecQualityDialog';
function ksExecQualityDialogW; external APPTOOL5 Name 'ksExecQualityDialogW';


function ksGetSelectedEmbodimentMarking; external APPTOOL5 Name 'ksGetSelectedEmbodimentMarking';
function ksGetSelectedEmbodimentMarkingW; external APPTOOL5 Name 'ksGetSelectedEmbodimentMarkingW';
function ksGetSelectedEmbodimentAdditionalNumber; external APPTOOL5 Name 'ksGetSelectedEmbodimentAdditionalNumber';
function ksGetSelectedEmbodimentAdditionalNumberW; external APPTOOL5 Name 'ksGetSelectedEmbodimentAdditionalNumberW';

function ksGetSpcPropertyFill; external APPTOOL5 Name 'ksGetSpcPropertyFill';
function ksSetSpcPropertyFill; external APPTOOL5 Name 'ksSetSpcPropertyFill';

// функции появившиеся в Компас17
function ksGetObjectsNameByType; external APPTOOL5 Name 'ksGetObjectsNameByType';
function ksGetObjectsNameByTypeW; external APPTOOL5 Name 'ksGetObjectsNameByTypeW';
function ksGetObjectNameByType; external APPTOOL5 Name 'ksGetObjectNameByType';
function ksGetObjectNameByTypeW; external APPTOOL5 Name 'ksGetObjectNameByTypeW';
function ksSetLightObjType; external APPTOOL5 Name 'ksSetLightObjType';
function ksShowHideTmpObj; external APPTOOL5 Name 'ksShowHideTmpObj';


// функции появившиеся в Компас18
function ksSetMixDlgMaterialParam; external APPTOOL5 Name 'ksSetMixDlgMaterialParam';
function ksSetMixDlgMaterialParamW; external APPTOOL5 Name 'ksSetMixDlgMaterialParamW';
function ksGetDocumentSaveVersion; external APPTOOL5 Name 'ksGetDocumentSaveVersion';
function ksGetDocumentOpenVersion; external APPTOOL5 Name 'ksGetDocumentOpenVersion';
function ksDistanceCurveCurve; external APPTOOL5 Name 'ksDistanceCurveCurve';

// функции появившиеся в Компас19
function ksSetLibraryEnable; external APPTOOL5 Name 'ksSetLibraryEnable';
function ksGetLibraryStatus; external APPTOOL5 Name 'ksGetLibraryStatus';
function ksIsLibraryTrial; external APPTOOL5 Name 'ksIsLibraryTrial';
function ksIsLibraryLocal; external APPTOOL5 Name 'ksIsLibraryLocal';
function ksIsLibraryProductName; external APPTOOL5 Name 'ksIsLibraryProductName';
function ksIsLibraryProductNameW; external APPTOOL5 Name 'ksIsLibraryProductNameW';
function ksIsLibraryProductKeyInfo; external APPTOOL5 Name 'ksIsLibraryProductKeyInfo';
function ksIsLibraryProductKeyInfoW; external APPTOOL5 Name 'ksIsLibraryProductKeyInfoW';
function ksIsPrintAvailable; external APPTOOL5 Name 'ksIsPrintAvailable';
function ksIsExportAvailable; external APPTOOL5 Name 'ksIsExportAvailable';
function ksRegisterLibraryNumber; external APPTOOL5 Name 'ksRegisterLibraryNumber';
function ksUnRegisterLibraryNumber; external APPTOOL5 Name 'ksUnRegisterLibraryNumber';

// функции появившиеся в Компас20
function ksSetZoomScale; external APPTOOL5 Name 'ksSetZoomScale';

// функции появившиеся в Компас21
function ksGetSpcObjectSummaryCountText; external APPTOOL5 Name 'ksGetSpcObjectSummaryCountText';
