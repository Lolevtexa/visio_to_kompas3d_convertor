...
//  Пересечение двух кривых.
//---------------------------------------------------------------------------------
void       LIB_FUNC    IntersectCurvCurvEx ( reference p1,             //указатель на первую кривую
                                             reference p2,             //указатель на вторую кривую
                                             int * kp,                 //количество точек пересечения
                                             double xp[], double yp[], //массивы точек пересечения
                                             int maxCount,             //максимально-допустимое количество точек в массивах
                                             int touchInclude );      // учитывать точки касания           
...
//   -7 ошибка
//---------------------------------------------------------------------------------
int  LIB_FUNC ksIntersectCurvCurvEx ( reference p1,       // указатель на первую кривую
                                      reference p2,       // указатель на вторую кривую
                                      reference array,    // динамический массив точек пересечения MathPointParam
                                      int touchInclude );// учитывать точки касания 
...
//   Возвращает:
//   1 успешное завершение
//   0 кривые пересекаются или совпадают растояние == 0.0
//   -1 первый объект не существует
//   -2 второй объект не существует
//   -3 кривые расположены в разных видах
//   -4 не совпадают СК определения кривых (геом и анн) (?)
...
//   -7 ошибка
//---------------------------------------------------------------------------------
int  LIB_FUNC ksDistanceCurveCurve( reference p1,            // указатель на первую кривую
                                    reference p2,            // указатель на вторую кривую
                                    double    * distanse,    // растояние
                                    double    * t1,          // параметр на кривой 1 в точке с минимальным удалением
                                    double    * t2  );       // параметр на кривой 2 в точке с минимальным удалением
...
// В случае неудачи возвращается -1
// ---
double LIB_FUNC  ksGetCurvePerpendicularByT( reference curve, // указатель на кривую
...
// Функция возвращает 1 в случае успеха или 0 при неудаче.
// ---
int LIB_FUNC  ksMovePointOnCurveEx( reference curve,      // указатель на кривую
                                    double *x, double *y, // координаты точки
                                    double *t,            // параметр кривой
                                    double len,           // расстояние на которое нужно сместить точку
                                    int    dir,           // направление продвижения точки(1 - в направлении построения кривой, -1 в обратном напрвлении )
                                    int    ext );         // 1 - Если кривая не замкнута и длина ее части от точки до конца в заданном направлении меньше, чем нужное смещение, то вычислить значение на продолжении кривой, если можно построить продолжение.
...
//-----------------------------------------------------------------------------
// Функция расчета касательных к кривой под заданным углом.
// ---
