...
// ---
  ...
  ViewParam = record
    ...
    name  : array[0..TEXT_LENGTH-1] of AnsiChar; // имя вида
  end;
...
// ---
  ...
  LayerParam = record
    ...
    name  : array[0..TEXT_LENGTH-1] of AnsiChar; // имя слоя
  end;
...
// ---
  ...
  RequestInfo = record
    title        : PAnsiChar;   // строка или идентификатор строки заголовка окна
    commands     : PAnsiChar;   // строка или идентификатор меню состава команд
    prompt       : PAnsiChar;   // строка или идентификатор приглашения
    cursor       : PAnsiChar;   // строка с именем или идентификатор стандартного курсора
...
// ---
  ...
  TFilePreviewFuncCallBack = function( fileName: PAnsiChar;  // идентификатор окна просмотра
			      	                         HWindow: HWND
...
TextItemFont = record
  fontName     : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя фонта
...
TextItemParam = record
  ...
  s        : array [0..TEXT_LENGTH-1] of AnsiChar; // массив символов  для компоненты текста
...
PSheetPar = ^SheetPar;
SheetPar = record
  layoutName : array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // для чертежа -имя  библиотеки оформления( штампа); пустая строка - GRAPHIC.LYT
...
DocumentParam = record
  ...
  fileName   : array [0..MAX_TEXT_LENGTH-1] of AnsiChar;  // имя  файла чертежа
  comment    : array [0..TEXT_LENGTH-1]     of AnsiChar;  // комментарий
  author     : array [0..TEXT_LENGTH-1]     of AnsiChar;  // автор
  layoutName : array [0..MAX_TEXT_LENGTH-1] of AnsiChar;  // имя  файла чертежа
...
ColumnInfo = record
  header          : array [0..80-1] of AnsiChar;          // заголовoк-комментарий столбца
  ...
  def             : array [0..TEXT_LENGTH-1] of AnsiChar; // значение по умолчанию
...
ksAttributeType = record
  ...
  key4          : Cardinal;     // системный код атрибута
                                ...
                                // за АО "АСКОН"
  header        : array [0..80-1] of AnsiChar; // заголовoк-комментарий типа
  rowsCount     : Cardinal;                // кол-во строк в таблице
  flagVisible   : Byte;                    // видимый, невидимый   в таблице
  password      : array [0..10-1] of AnsiChar; // пароль, если не пустая строка  - защищает от несанкционированного изменения типа
...
Attribute = record
  ...
  key4        : Cardinal;     // системный код атрибута
                              // значения от 0 до 1000 зарезервированы
                              // за АО "АСКОН"
  flagVisible : PAnsiChar;        //массив, определяющий для каждой колонки
  ...
  password    : array[0..10-1] of AnsiChar; // пароль
end;
...
ksAttribute = record
  ...
  values      : Pointer;               // массив значений ячеек таблицы атрибутов
                              ...
                              // затем все значения для 2-ой строки и т.д.
  valSize     : Cardinal;     // размер массива значений ячеек
  password    : array[0..10-1] of AnsiChar; // имя вида
  columnKeys  : PAnsiChar;        // массив ключей колонок
end;
...
LibraryAttrTypeParam  = record
  name     : array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя  типа атрибута
...
BaseParam = record
  ...
  0 : ( str      : array[0..50-1] of AnsiChar );// надпись
...
CurvePatternEx = record
  ...
  1 : ( frwName : array[0..TEXT_LENGTH-1] of AnsiChar);//pictureType 1  картинка в виде фрагмента, используется только при создании стиля AddStyle
end;
...
CurveStyleParam = record
  name 	      : array [0..TEXT_LENGTH-1] of AnsiChar; // имя стиля
  color       : DWord;                          // цвет
...
TextStyleParam = record
  ...
  fontName   :  array [0..TEXT_LENGTH-1] of AnsiChar; //имя фонта
  color      :  DWord;                          //цвет
...
PLibStyle = ^LibStyle;
LibStyle = record
  fileName 	 :  array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла библиотеки
...
SpecRoughParam = record
  ...
  s 	 : array [0..TEXT_LENGTH-1] of AnsiChar; //текст
end;
...
SpcColumnParam = record
  ...
  name 	     : array [0..TEXT_LENGTH-1] of AnsiChar; // имя колонки
end;
...
LibraryStyleParam = record
  styleName : array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя  стиля
...
VariableParam = record
  name  : array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя  переменной
  value : Double;                             // значение переменной
  note  : array [0..MAX_TEXT_LENGTH-1] of AnsiChar; // комментарий
end;
...
InsertFragmentParam = record
  ...
  fileName   : array [0..MAX_TEXT_LENGTH-1] of AnsiChar;  // имя файла фрагмента или "\0" для локального фрагмента
  comment    : array [0..MAX_TEXT_LENGTH-1] of AnsiChar;  //имя вставки
end;
...
DocAttachedSpcParam  = record
  fileName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла документа источника
  comment  : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // комментарий для документа источника
...
SpcObjParam  = record
  ...
  subSectionName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя подраздела !!! только для чтения !!!
...
RasterParam  = record
  ...
  fileName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // полный путь к файлу
...
RecordTypeAttrParam  = record
  attrLibName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar;  // имя файла библиотеки типов атрибутов
...
SpcStyleColumnParam  = record
  nameColumn : array[0..MAX_TEXT_LENGTH-1] of AnsiChar;  // имя колонки
  ...
  rezerv : array[0..9] of AnsiChar; // не используется
...
SpcStyleSectionParam  = record
  sectionName    : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя раздела
...
SpcSubSectionParam  = record
  name   : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; //имя подраздела
...
SpcTuningStyleParam  = record
  ...
  rezerv      : array[0..3] of AnsiChar; // не используется
  ...
  predefinedTextFileName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar;
...
SpcStyleParam  = record
  layoutName1 : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла библиотеки оформлений для первого листа
  layoutName2 : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла библиотеки оформлений для второго листа
...
SpcDescrParam  = record
  layoutName : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла библиотеки стилей
  styleId    : Cardinal;                            // номер стиля в библиотеке
  spcName    : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя подключенного файла СП
end;
...
QualityContensParam = record
  ...
  name          : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // поле допуска
...
RasterFormatParam = record
  ...
  pages         : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // список диапозонов выводимых листов  "beg1-end1, beg2-end2, beg3-end3,  ..."
...
PolylineParamEx = record
  ...
  rezerv     : array[0..10] of AnsiChar;       // не используется
end;
...
TreeNodeParam = record
  ...
  name        : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя узла
...
AssociationViewParam  = record
  ...
  fileName       : array[0..MAX_TEXT_LENGTH-1] of AnsiChar; // имя файла 3d документа
  projectionName : array[0..TEXT_LENGTH-1    ] of AnsiChar; // имя проекции ( из списка проекций в документе источнике )
...
CopyObjectParam  = record
  ...
  reserv         : array[0..7] of Byte;  // резерв - не используется
end;
...
//-----------------------------------------------------------------------------
// Структура параметров библиотечной панели свойств
// ---
PLibToolbarSettings = ^LibToolbarSettings;
LibToolbarSettings = record
  defaultVisible : Byte;               //  0 погашена, по умолчанию
  docsType       : OleVariant;         // safearray констант типов документов, в которых панель видна (если не погасил пользователь вручную)
end;
