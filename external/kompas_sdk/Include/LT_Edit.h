...
// с указателем ref (объект вида).
// Возвращает 1 в случае успеха, 0 в случае неудачи.
//-----------------------------------------------------------------------------
int     LIB_FUNC  ksSetLightObjType( reference ref,         // указатель на объект
                                     unsigned char light ); // режим подсветки
...
// Возвращает 1 в случае успеха, 0 в случае неудачи.
//-----------------------------------------------------------------------------
int     LIB_FUNC ksShowHideTmpObj( reference ref,    // указатель на объект
                                   int       show ); // ksShowHideTmpObjTypeEnum скрыть-показать, показать как фантом 
...
reference  LIB_FUNC DecomposeObj( reference p,        //указатель на объект
                                unsigned char level,  //уровень разбиения 0 - отрезки,дуги,тексты,точки;
                                                      ...
                                                      //                  7 - составные объекты (макро, контур, вставка фрагмента) на составляющие объекты, простые объекты - копии 
...
#ifdef _UNICODE
#define ksSetMixDlgMaterialParamT  ksSetMixDlgMaterialParamW
#else
#define ksSetMixDlgMaterialParamT  ksSetMixDlgMaterialParam
#endif // !UNICODE
...
//  и масштабом . Координаты центра в СК листа
//------------------------------------------------------------------------------------------------
...
//  растянуть изображение активного окна на весь экран, ограниченное прямоугольником с центром
//  и масштабом . 
// viewParam - TRUE - Координаты центра в СК текущего вида
// viewParam - FALSE - Координаты центра в СК листа
//------------------------------------------------------------------------------------------------
int  LIB_FUNC ksSetZoomScale ( double x, double y, double scale, int viewParam );
...
// ---
int LIB_FUNC ksGetRecoverModeErrorList( VARIANT * errors);
...
#ifdef _UNICODE
#define ksCreateInsertionFragmentT  ksCreateInsertionFragmentW
#else
#define ksCreateInsertionFragmentT  ksCreateInsertionFragment
#endif // !UNICODE


// Экспортный номер - @1102
//-------------------------------------------------------------------------------
...
// Экспортный номер - @1103
//-------------------------------------------------------------------------------
// Версия файла с которой документ был сохранен
//-------------------------------------------------------------------------------
int ksGetDocumentOpenVersion( reference docRef );
